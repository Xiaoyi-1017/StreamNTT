"""shiftadd_generator.py - shiftadd header/arithmetic generation for StreamNTT.

Extracted verbatim (COPY-first, behavior-preserving) from generate_code.py.
Do not edit arithmetic here. See Spec/helper_extraction_plan_streamntt_shiftadd.md.
"""
...
def _shiftadd_q_anchor(aliases_q):
    ...
    exponent / shared fold shift M = 2**B) is chosen by a TEMPORARY heuristic from {51, 61}
    (see below), NOT min(K_i). B is NOT K_env (the HLS Data2 container width) and NOT K_group.
    Only K_spread < 3 (compatible-width) groups are accepted; wider spreads raise.
    """
    # --- B-anchor selection: TEMPORARY current-table heuristic (DEBUG; to be corrected later) ---
    # TEMP DEBUG: B is chosen from the known {51, 61} anchor families by q.bit_length(), instead of
    # min(q.bit_length()). For SPARSE known primes q = 2**e +/- (small terms), q.bit_length() can be
    # e+1, so min(bit_length) picks the wrong solo anchor and the fold/correction recipe becomes
    # unservable. Examples:
    #     TI_9  = 2**61 + 2**23 + 2**21 + 1  -> bit_length 62, but the anchor B should be 61.
    #     TII_33                             -> bit_length 52, but the anchor B should be 51.
    # This heuristic ONLY covers the current 51/52/61/62 prime families. Do NOT generalize it to
    # arbitrary external primes. If additional anchor families appear later, REPLACE it with an
    # explicit function/metadata recognizer: primality + 2N NTT-friendliness, sparse-form
    # recognition, and lightweight-prime recipe eligibility.
    ...
    # TEMP DEBUG heuristic: map each prime's bit_length to a known anchor in {51, 61}; B = min over batch.
    B = min(51 if q.bit_length() <= 52 else 61 for _alias, q in aliases_q)
    return B, K_group, K_spread


def _resolve_anchor(aliases_q, recipe=None):
    """Phase 3.5 S3c anchor source-of-truth (transition assert-shim seam).
    ...
    invoked ONLY to assert agreement (the S3b transition guard already pins
    ...
    if recipe is not None and getattr(recipe, "batch_legal_shiftadd", False):
        ...
        if got != want:
            raise RuntimeError(
                "shiftadd S3c anchor drift: _shiftadd_q_anchor %r != recipe "
...
# ---------------------------------------------------------------------------
# K_group mixed-prime (two-anchor-family) emission
# ---------------------------------------------------------------------------
# User K_group semantics (binding): "same-bit" means same K_group / arithmetic
# recipe group, NOT identical q.bit_length() (51/52-bit TII primes are ONE
# K_group=52 family; 61/62-bit TI primes are the K_group=62 envelope). A batch
# is "mixed" for shift-add generation ONLY when it spans BOTH known anchor
# families (fold/mul bases {51, 61} == K_groups {52, 62}); such a batch emits
# ONE reducer containing TWO class pipelines (each the proven single-family
# recipe for its class sub-batch) and a cls-based result select
# (cls = PRIME_CLASS[mod_id]; ntt_primes.h is included before ntt_shiftadd.h).
# For the only legal mixed map {52, 62} the emitted absolute PRIME_CLASS rule
# (0 = 52-class, 1 = 62-class) equals the set-relative cls0/cls1 order, so
# `cls ? <c1> : <c0>` selects correctly. The inactive class pipeline computes
# a discarded value (defined ap_int truncation; selected away by the cls mux)
# -- the same dual-candidate + cls-mux principle as the Barrett mixed-bit
# reduce block. Single-family batches NEVER enter this path (legacy emitters
# stay byte-identical); custom/unknown bit lengths fall through to the legacy
# path and its existing behavior.
...
def _emit_shiftadd_reduce_header_mixed(out_path, K_env, aliases_q, header_top,
                                       ...
                                       *, recipe=None):
    """Emit reduce_shiftadd(...) for a K_group-mixed {52, 62} known batch.

    Every per-class block reuses the SINGLE-FAMILY machinery on its class
    sub-batch (auto fold/correction resolver, q-derived envelope widths, math
    self-check, slot assignment) and emits the proven slot-direct fold
    pipeline with a ``_c0`` / ``_c1`` name suffix and its own correction; the
    final result is ``cls ? r_out_c1 : r_out_c0``. Per-prime ``is_<alias>``
    predicates keep GLOBAL mod_id indices, so each class's slot muxes select
    per-prime candidates exactly as in the single-family emitter.

    Recipe consumption (S3c discipline): when ``recipe.cls_recipes`` is
    ...
    combined_log = []
    classes = []
    for cls_idx, (k_class, B, sub) in enumerate(subs):
            ...
            if (cr.b_anchor != B) or ((fold_c, corr_c) != want):
                raise RuntimeError(
                    ...
                    "(B, fold, correction) %r != recipe %r" % (
                        cls_idx, k_class, (B, fold_c, corr_c),
                        (cr.b_anchor,) + want))
            fold_c, corr_c = want
        ...
        if not all_FN:
            ...
            raise RuntimeError(
                f"reduce_shiftadd (mixed cls{cls_idx} {k_class}-class): not "
        ...
        if not ok_math:
            raise RuntimeError(
                f"math self-check FAILED for mixed cls{cls_idx} "
        ...
        classes.append(dict(
            cls_idx=cls_idx, k_class=k_class, B=B, fold=fold_c, corr=corr_c,
    ...
    def _class_pipeline(c):
        ...
        pre = "ShiftaddPreC%d" % cls_idx
        L.append(f"    // cls{cls_idx} correction ({corr})")
        ...
        L.append(f"        r{tag} -= q_ext{tag};")
        L.append("    }")
        L.append(f"    Data r_out{tag} = (Data)r{tag};")
        return L

    lines = list(header_top)
    lines.append("// Generated shift-add reducer for this K_group-mixed prime batch.")
    lines.append("// Two class pipelines selected by cls = PRIME_CLASS[mod_id]:")
    lines.append("//   cls == 0: 52-class (51/52-bit family), fold base B = 2^51.")
    lines.append("//   cls == 1: 62-class (61/62-bit family), fold base B = 2^61.")
    ...
    for i in range(NP):
        lines.append(f"    const bool is_{aliases_clean_all[i]} = "
                     f"(mod_id == (ModId){i});")
    lines.append("")
    lines.append("    // Per-class recipe select: the absolute PRIME_CLASS rule")
    lines.append("    // (0 = 52-class, 1 = 62-class) equals the set-relative cls")
    lines.append("    // order for the only legal mixed map {52, 62}.")
    lines.append("    const int cls = PRIME_CLASS[mod_id];")
    lines.append("")
    for c in classes:
        lines.extend(_class_pipeline(c))
        lines.append("")
    lines.append("    return cls ? r_out_c1 : r_out_c0;")
    lines.append("}")
    lines.append("")
    lines.append("#endif  // NTT_SHIFTADD_H")
    lines.append("")
...
def _emit_shiftadd_mul_header(out_path, K_env, aliases_q, header_top,
    ...
    # S3c: on the recipe-governed path base = recipe.b_anchor (assert-shimmed);
...
def _emit_shiftadd_mul_header_mixed(out_path, K_env, aliases_q, header_top,
    ...
    recipe is informational here, matching the single-family S3c choice).
    """
    subs = _shiftadd_mixed_guard(aliases_q)
    NP = len(aliases_q)
...
def emit_shiftadd_header(case_dir, K_env, aliases_q, enabled,
    ...
    Phase 3.5 S4: ``single_prime_mode`` in {"legacy" (default), "normalized"}.
    ...
    # K_group mixed-prime dispatch: a batch spanning BOTH known anchor
    # families ({51, 61} == K_groups {52, 62}) routes to the per-class mixed
    # emitters. Single-family and custom/unknown-bit-length batches take the
    # legacy single-anchor path below byte-identically.
    _mixed_families = (_shiftadd_known_anchor_families(aliases_q) == [51, 61])
    ...
    # S3c: the resolver is DETERMINISTIC and recipe.reduce_resolver_params is the
    ...
    if recipe is not None and getattr(recipe, "batch_legal_shiftadd", False):
        ...
        if (resolved_fold_depth, resolved_correction) != want:
            raise RuntimeError(
                "shiftadd S3c resolver drift: live (fold, correction) %r != recipe "
