"""Shift-add reduction recipe composition.

Defines immutable per-prime facts and per-prime-set recipes derived from the
known prime table and the pure arithmetic helpers in ``reduce_generator`` and
``shiftadd_generator``. This module never imports the top-level generator;
the shift-add emitter consumes recipe instances without a reverse import.

Reduction capability records two distinct results: the fixed default
``(fold_depth=2, positive_only)`` self-check and the live resolver result used
by the emitter. A failure at the fixed point does not imply that the live route
is unsupported. Multiplication capability records its independently verified
correction count.

``anchor_k_group`` is the shift-add anchor-side batch width
``max(q.bit_length())``. It is distinct from the reduction class ``K_group``
and from ``final_K``.
"""
...
# The fixed default reduce-route evaluation point matches the
...
# Bit lengths accepted by the anchor heuristic (_shiftadd_ell).
SHIFTADD_ANCHOR_BIT_LENGTHS: Tuple[int, ...] = (31, 32, 51, 52, 61, 62, 63, 64)

# Validity frame for the recorded facts.
...
# Curated overrides are unsupported; a populated hook fails at construction.
...
class PrimeReduceFacts:
    """Per-alias reduce/mul route facts.
    ...
    with t_match False means supported with correction, never unsupported.
    ...
    rel_class: Optional[int]                 # set-relative index into the recipe class_map
    anchor_exp: Optional[int]                # 51 | 61, stored independently; from _shiftadd_ell
    provenance: str                          # "known_table" | "unsupported_custom"
    #                                          ("future_recognized" for trusted generated primes)
...
class ClassShiftaddRecipe:
    ...
    matching the single-family emitter choice).
    """
    k_class: int                              # 52 | 62 reduction class
...
class ReduceRecipe:
    ...
    that the existing emit path would refuse.
    """
    aliases: Tuple[str, ...]
    ...
    #                                          max q.bit_length(). Not the reduction-class K_group
    #                                          (max K_arith) and NOT final_K.
    k_spread: int                            # max - min q.bit_length()
    class_map: Tuple[int, ...]               # classes present, ascending; () if unavailable
    ...
    # Route capabilities exposed by the current emitters.
...
def _provenance_for_alias(alias: str) -> str:
    ...
    convention) are custom primes: recorded here rather than raised."""
...
def _alias_facts(alias: str, q: int, k_env: int) -> PrimeReduceFacts:
    ...
    if ell2 != anchor_exp:
        raise RuntimeError(
            "reduce_recipe consistency guard: solo envelope anchor %r != "
...
def build_reduce_recipe(aliases_q: Sequence[Tuple[str, int]], *,
                        ...
                        classify_info: Optional[ReduceClassInfo] = None,
                        trusted_generated_q=frozenset()) -> ReduceRecipe:
    """Build a pure, immutable recipe from known-table arithmetic facts.

    Derives every fact from q via the existing helpers, stores
    anchor_exp independently, and asserts batch equivalence to the
    ...
    batch_legal_shiftadd=False. When some anchor is unknown (custom
    ...
    (the recipe subsumes both helpers).
    ...
    provenance "unsupported_custom" is recorded without raising. Raises only on structurally
    invalid input, a populated override hook, or a violated consistency
    ...
    # Trusted primes produced and recognized during this run earn the
    # "future_recognized" provenance, which the shift-add gate does
    # NOT block. ONLY primes currently labelled "unsupported_custom" (i.e. INT_<q>) AND whose q is in
    # the caller-supplied trusted set are upgraded -- arbitrary user INT_<q> primes (absent from the
    # set) stay "unsupported_custom" and remain blocked from shift-add. Known-table primes are never
    # touched. Default (empty set) -> no change -> byte-identical to every existing caller.
    if trusted_generated_q:
        primes = tuple(
            replace(f, provenance="future_recognized")
            if (f.provenance == "unsupported_custom" and f.q in trusted_generated_q) else f
            for f in primes)
    ...
    # Consistency guard against the current batch heuristic.
    if known_two_family:
        ...
        for _k_cls, _b_cls, _sub in _shiftadd_class_sub_batches(pairs):
            ...
            if heur[0] != _b_cls:
                raise RuntimeError(
                    "reduce_recipe consistency guard: class %d sub-batch "
                    "anchor %r != %d for %r"
        ...
        try:
            ...
            if ("K_spread=%d >= 3" % k_spread) not in str(exc):
                raise RuntimeError(
                    "reduce_recipe consistency guard: unexpected "
                    "_shiftadd_q_anchor reason for k_spread=%d: %s"
                    % (k_spread, exc))
        else:
            raise RuntimeError(
                "reduce_recipe consistency guard: _shiftadd_q_anchor accepted "
        ...
        if heur != (b_anchor, anchor_k_group, k_spread):
            raise RuntimeError(
                "reduce_recipe consistency guard: (b_anchor, anchor_k_group, "
    ...
    # the recipe records the conservative _shiftadd_ell view.
    ...
    elif any(f.provenance == "future_recognized" for f in primes):
        provenance = "future_recognized"
    else:
