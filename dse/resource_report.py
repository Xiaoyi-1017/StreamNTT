"""Pre-generation resource REPORT assembler / renderer (Phase 4.0 Slice 3c-2) -- report-only.
...
REPORT_CAVEATS = (
    ...
    "1410 = x_stages + TFR-X is the audited RapidStream group anchor, not a universal merge rule.",
    ...
    "BU8 DSP profile (240,270,330,450) is an audited anchor, not a universal split result.",
...
def build_report(N: int, BU: int, CH: int, K: int, route: str,
    ...
    """Assemble a ResourceReport from the committed resource_model estimators. report-only.
    ...
    DEVICE guard (e.g. the audited 3090); tfr_x_dsp/tfg_x_dsp/extra_group_dsp feed the GROUP estimate.
    """
    config = {
...
def render_report(report: ResourceReport) -> str:
    ...
    if report.memory is not None:
        mem = report.memory
        out.append("[memory]   ICBU URAM=%d BRAM_18K=%d (%s; bram=%s; audited N1024/BU8/2U2B anchor)"
