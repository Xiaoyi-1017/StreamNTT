"""Calibration comparison layer for the pre-gen cost model (Phase 4.0 Slice 4b) -- report-only.

Compares resource_model PREDICTIONS against calibration_data OBSERVED records, producing deterministic
ComparisonRecords + a text report. This is the FIRST module that imports both layers:
    calibration_compare -> calibration_data   (observed evidence)
    calibration_compare -> resource_model      (predictions)
(calibration_data must NOT import resource_model.) NOT wired to generate_code.py; no DSE apply; no X-split.
...
def render_comparison_report(comparisons) -> str:
    ...
    out.append("[caveats]  N131072 = shiftadd=mul SUBMODEL/historical (ICBU DSP/memory), NOT full-design or "
