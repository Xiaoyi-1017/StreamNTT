...
/* Changed to prevent overflow */
void sw_ntt(std::vector<HostData, tapa::aligned_allocator<HostData>> A, std::vector<HostData, tapa::aligned_allocator<HostData>> &out_sw, HostData psi_val, HostData q, int poly_start, int poly_end){
...
#pragma omp parallel for
    for (int p = poly_start; p < poly_end; ++p) {
        ...
        for (int j = 0; j < n; ++j) {
        	HostData psi_j = (HostData) mod_power(psi_val, j, q);
...
int main(int argc, char* argv[]) {
	...
	for (int mid = 0; mid < NUM_PRIMES; ++mid) {
		// Collect range of polys that use this mod_id
		// For cyclic: poly mid, mid+NUM_PRIMES, mid+2*NUM_PRIMES, ...
		HostData q_mid = (HostData)MODS[mid];
		HostData psi_mid = PSIS[mid];
		for (int p = mid; p < ADJ_POLY_NUM; p += NUM_PRIMES) {
			sw_ntt(input, out_sw, psi_mid, q_mid, p, p+1);
		}
	}
