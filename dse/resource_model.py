"""Pre-generation resource cost model (Phase 4.0 Slice 2) — read-only / report-only.

Encodes the VERIFIED structural formulas (Slice 1) and the CALIBRATED per-modular-
reduction DSP coefficient table D_MR (Slice 1b) for the StreamNTT segmented mainline.
Pure functions, no I/O, no generation, no apply. It NEVER changes a recipe/route/config.

Scope / evidence (recorded in Checkpoint/phase4_0_pregen_cost_model_formula_audit_*.md
and ..._dmr_provenance_audit_*.md):
...
Caveats carried from the audits:
...
TFS_IDSKIP = "idskip"           # HISTORICAL lane-level model (one per non-identity LANE); NON-DEFAULT
...
# --- structural formulas (Slice 1; verified vs emitted headers + csynth) ----
...
def _tfs_per_stage(BU: int, tfs_model: str) -> Tuple[int, ...]:
    ...
    if tfs_model == TFS_IDSKIP:
        # historical lane-level: one per non-identity LANE: BU - BU/2**s
...
def estimate_x_stage_mr_count(BU: int, tfs_model: str = TFS_GROUPSHARE) -> int:
    ...
    tfs_model=TFS_IDSKIP gives the historical lane-level count (49 @ BU8)."""
...
# --- calibrated D_MR table (Slice 1b; csynth N1024/BU8; provenance-tagged) ---
...
_DMR_TABLE: Dict[Tuple[str, int, str], DmrCoefficient] = {
    (ROUTE_BARRETT, 52, "single"): _entry(15, ROUTE_BARRETT, 52, "single", "s1b_cfg2 (single TII_33)", "csynth"),
    (ROUTE_BARRETT, 52, "pair"):   _entry(17, ROUTE_BARRETT, 52, "pair", "p32_xo_2u2b / s1b_cfg4; x_stages 731", "csynth+rapidstream"),
    (ROUTE_BARRETT, 61, "pair"):   _entry(23, ROUTE_BARRETT, 61, "pair", "tfs_groupshare_bu8_post (61-bit pair); x_stages 989", "csynth"),
    (ROUTE_BARRETT, 62, "single"): _entry(26, ROUTE_BARRETT, 62, "single", "s1b_cfg1 (single TI_9)", "csynth"),
    (ROUTE_BARRETT, 62, "pair"):   _entry(30, ROUTE_BARRETT, 62, "pair", "stepc1 (TI_9+61bit); x_stages 1290 == RapidStream forensic", "csynth+rapidstream"),
    # shiftadd routes: DSP is the shared U product only -> route-flat in prime count (single==pair)
    (ROUTE_SHIFTADD_REDUCE, 52, "single"): _entry(6, ROUTE_SHIFTADD_REDUCE, 52, "single", "s4_tii33_red", "csynth"),
    (ROUTE_SHIFTADD_REDUCE, 52, "pair"):   _entry(6, ROUTE_SHIFTADD_REDUCE, 52, "pair", "s4_tii33_red (route-flat)", "csynth"),
    (ROUTE_SHIFTADD_REDUCE, 62, "single"): _entry(11, ROUTE_SHIFTADD_REDUCE, 62, "single", "s4_ti9_red", "csynth"),
    (ROUTE_SHIFTADD_REDUCE, 62, "pair"):   _entry(11, ROUTE_SHIFTADD_REDUCE, 62, "pair", "step11_reduce; x_stages 473", "csynth+rapidstream"),
    (ROUTE_SHIFTADD_MUL, 52, "single"): _entry(6, ROUTE_SHIFTADD_MUL, 52, "single", "s4_tii33_mul", "csynth"),
    (ROUTE_SHIFTADD_MUL, 52, "pair"):   _entry(6, ROUTE_SHIFTADD_MUL, 52, "pair", "s4_tii33_mul (route-flat)", "csynth"),
    (ROUTE_SHIFTADD_MUL, 62, "single"): _entry(11, ROUTE_SHIFTADD_MUL, 62, "single", "s4_ti9_mul", "csynth"),
    (ROUTE_SHIFTADD_MUL, 62, "pair"):   _entry(11, ROUTE_SHIFTADD_MUL, 62, "pair", "s4_ti9_mul (route-flat)", "csynth"),
}
...
_DMR_KNOWN_UNKNOWN: Dict[Tuple[str, int, str], str] = {
    (ROUTE_BARRETT, 61, "single"): "61-bit single-prime Barrett not observed on disk (Slice 1b V1)",
}
...
# --- ICBU memory (Slice 3a; reuses icbu_dse as the storage estimator plugin) -
...
def estimate_icbu_memory(N: int, BU: int, CH: int, K: int, num_core: int = 1,
    ...
    DSP is intentionally NOT taken from icbu_dse here (its CALIB hardcodes DSP=30, the audited 62-bit-pair
    case); authoritative bf_unit DSP = estimate_l_stage_bf_dsp (D_MR-keyed).

    Audited anchor (provenance, NOT a universal constant): N1024/BU8/CH4/2U2B -> URAM 96 / BRAM_18K 192
...
def guard_status_for_dsp(dsp_value: int, guard: float = DEFAULT_DSP_GUARD) -> str:
    ...
    SLR/SLOT-level capacity (~1228), a SEPARATE layer modelled by slot_dsp_guard (Slice 3b).
    """
    from dse import icbu_dse  # lazy: keep the core model dependency-free
    return icbu_dse.guard_status(dsp_value, "DSP", icbu_dse.BOARD_U55C, guard)


# --- layered DSP guardrail (Slice 3b; reuses icbu_dse.guard_status for the status math) -------
#
...
# ever surfaces it is from an external P&R/report layer outside this audit and must never be silently
...
# --- per-stage / per-group X-stage DSP (Slice 3c-1; Phase 5 split-planning DATA ONLY) ---------
...
def estimate_x_stage_per_stage_dsp(BU: int, d_mr: int,
    ...
    calibrated/hypothetical coefficient). BU8 group-shared @ d_mr=30 -> (240,270,330,450), an AUDITED
    anchor, NOT a universal split result. D_MR BU-invariance is unproven, so any BU != 8 figure here is
    structural/formula output, not calibrated P&R truth. idskip (TFS_IDSKIP) is historical / non-default."""
...
def estimate_x_stage_partition_guard(stage_dsp, partition,
    ...
    """Slot-guard status per partition group (reuses slot_dsp_guard). Phase 5 DATA only: a group over the
