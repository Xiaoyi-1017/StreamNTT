...
resource / ICBU bind_storage DSE reporting layers. Three profiles:
  ...
  u55c : calibrated/default post-shell available capacity, not raw datasheet totals.
...
Only u55c has DSE calibration evidence from RapidStream/csynth measurements.
u250/u280 carry capacity numbers only:
...
class DeviceProfile(NamedTuple):
    ...
    def as_board(self) -> dict:
        """Return the board-capacity mapping consumed by icbu_dse."""
...
DEVICE_PROFILES = {
    "u55c": DeviceProfile(
        ...
        notes="calibrated default: post-shell available capacity; the only device with DSE calibration evidence "
