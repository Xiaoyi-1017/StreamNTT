"""ICBU bind_storage minor-DSE / resource-balancing guard (Phase 3.3.4 follow-up).

Estimates ICBU ping-pong bind_storage resource pressure (LUTRAM/BRAM/URAM) for the configured architecture,
compares against a configurable routability PROXY guard on a board profile, and RECOMMENDS a bind_storage
rebalancing ONLY when a resource is near/over guard. report/off only this slice; apply is GATED (Phase 3.3.4 K4,
handled in generate_code.py). It does NOT change the design here.

NOT array_partition / xo-opt / shiftadd-mux / reduce-path / X-stage-split / R-hetero / Layer-3 DSE. The guard is a
PROXY (default 0.80), NOT a hard physical guarantee and NOT the RapidStream ~86% slot-level threshold.
...
# U55C available resource profile -- derived from the device_profiles single source of truth
# (IDENTICAL values to the historical literal {"FF": 2167360, "LUT": 1045680, "BRAM_18K": 3552,
# "DSP": 8204, "URAM": 960}; equality locked by tests/test_device_profiles.py). F/L/B/D/U.
...
CALIBRATION_ROOT = "Checkpoint/dse_calibration"   # large-N calibration evidence (gitignored; may not exist).
...
# Candidate classes (policy plan rev-1): only `calibrated` recipes may become STRONG recommendations.
# closed-form = what-if; exploratory = uncalibrated until large-N LUT/FF evidence (4L: never strong without it).
RECIPE_CLASS = {"2U2B": "calibrated", "3U1B": "calibrated", "4U": "closed-form", "4B": "closed-form",
                "4L": "exploratory", "3U1L": "exploratory"}

# Empirical per-bf_unit module totals measured this session (LUT/FF compute-dominated; DSP recipe-invariant).
...
# Calibrated BRAM_18K per B bank from RapidStream vertex evidence (Checkpoint/dse_calibration, 2026-06-06).
...
class Recommendation(NamedTuple):
    ...
    rows: list
    calibrated: bool
    # DSE-apply slice: machine-readable decision fields (appended with defaults -> existing
    # 3-positional constructions/callers stay valid). `best` is set ONLY by a D2 strong
    # recommendation (calibrated background + calibrated-class candidate under guard on BOTH
    # mem resources); it is NEVER a guess. `decision` = the D0-D4 policy branch taken.
...
def recommend(arch: Arch, background: Optional[dict], board: dict, guard: float,
    ...
    """D0-D4 recommendation policy (policy plan rev-1). Report-only; NEVER auto-applies. STRONG recommendations
    ...
    if bg_over:
        return Recommendation(
            ...
            "suppressed (deferred: X-stage split / reduce-DSP / config reduction)" % "/".join(bg_over),
            rows, True,
    ...
    if strong:
        ...
        return Recommendation(
            ...
            "historical, max mem util %.1f%%, delay_risk %s)"
    ...
    return Recommendation(
        ...
        "calibrated candidates); deferred: R-hetero / config reduction / future validation; exploratory recipes "
        "(4L/3U1L) require explicit validation", rows, True,
...
def format_report(arch: Arch, calib_record: Optional[dict], board: dict, guard: float,
    ...
    `device` (default "u55c") only labels/annotates: u55c output is byte-identical to the historical
    form. For a non-u55c device the caller MUST pass that device's capacity `board` AND calib_record=None
    ...
    lines = ["=== ICBU bind_storage minor-DSE (report) ==="]
    ...
    lines.append("BRAM bank packing: %s | confidence: %s"
                 % (packing_display, "calibrated historical" if (rec.calibrated and packing == "calibrated")
